...
- [Claude Code & Pulu-workspace — Cấp độ chuyên sâu](#7-claude-code--pulu-workspace)
...
**Quan trọng:** Khi bạn **đóng tab / mở conversation mới** → Claude quên hết. Mỗi conversation là một trang giấy trắng mới — **trừ khi** dùng Pulu-workspace với hệ thống Memory.

### Claude KHÔNG tự động làm được:
- Nhớ bạn sau khi đóng hội thoại (trừ Pulu-workspace/Memory)
...
└── 🏢  Pulu-workspace               → Claude Code + Memory + Skills + MCP
```
...
❓ Tôi muốn Claude nhớ tôi, kết nối Gmail/Calendar/Drive, chạy task tự động
   → Pulu-workspace ✅ (cần setup — liên hệ Khanh)
```
...
### Pulu-workspace là gì?

**Pulu-workspace** = Claude Code + Memory + Skills + MCP Tools — biến Claude từ "chatbot thông minh" thành **trợ lý làm việc tích hợp thực thụ**.

> Hình dung: `claude.ai` = xe máy đi được.  
> `Pulu-workspace` = xe máy đã lắp thêm GPS, hộp đồ, kết nối Bluetooth, camera — cùng động cơ nhưng mạnh hơn nhiều.

### So sánh đầy đủ các nền tảng

| Tính năng | claude.ai | Projects | Claude Desktop | Pulu-workspace |
|---|---|---|---|---|
| Chat hỏi đáp | ✅ | ✅ | ✅ | ✅ |
| Upload file để đọc | ✅ | ✅ | ✅ | ✅ |
...
Đây là phần **khác biệt lớn nhất** của Pulu-workspace so với claude.ai. Pulu-workspace hoạt động dựa trên một hệ thống **file văn bản** (.md = Markdown) mà Claude đọc mỗi khi bắt đầu session.

### Tổng quan cấu trúc thư mục Pulu-workspace

```
Desktop/Pulu-workspace/
│
...
Đây là điều **ít người hiểu rõ nhất** — cách một session Pulu-workspace thực sự diễn ra từng bước.
...
- **Bước 1-2 (Context loading):** Đây là lý do Pulu-workspace "thông minh hơn" claude.ai — nó bắt đầu với đầy đủ bối cảnh thay vì trang trắng
...
### Skills hiện có trong Pulu-workspace
...
### MCP Tools đang kết nối trong Pulu-workspace
...
| **Pulu-workspace** | Môi trường đầy đủ: Claude Code + Memory + Skills + MCP |
...
| **Skills** | Lệnh tắt chuyên dụng trong Pulu-workspace — gõ `/tên-skill` |
...
**Người dùng nâng cao (cần setup Pulu-workspace):**
- [ ] Setup Pulu-workspace với Khanh (liên hệ trực tiếp)
...
*Câu hỏi hoặc góp ý setup Pulu-workspace: Khanh — khanhlp@ahamove.com*
*Cập nhật: 2026-05-08*
