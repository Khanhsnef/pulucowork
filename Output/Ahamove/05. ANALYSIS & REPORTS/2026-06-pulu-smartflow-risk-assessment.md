...
4.  **Tự động đồng bộ hóa (Auto-sync workflow):** Hook `Stop` trong tệp cấu hình [.claude/settings.json](file:///Users/ts-1148/Desktop/Pulu-workspace/.claude/settings.json#L8) tự động chạy chuỗi lệnh Git commit & push kèm theo rsync đè thư mục tới Google Drive của Lê Phương Khanh (`lephuongkhanh1995@gmail.com`).
...
*   **Xác suất Rò rỉ Dữ liệu Chiến lược (Confidence Interval: 90%):** Do hook `Stop` trong [.claude/settings.json](file:///Users/ts-1148/Desktop/Pulu-workspace/.claude/settings.json#L8) thực hiện rsync tự động đè thư mục, nếu file `.gitignore` không loại trừ kỹ các file tạm, toàn bộ lược sử chat chứa chiến lược tái cơ cấu Driver Tiering 2026, thông tin cá nhân tài xế và dữ liệu P&L/SLA Ahamove sẽ bị đồng bộ lên kho lưu trữ đám mây cá nhân/GitHub.
