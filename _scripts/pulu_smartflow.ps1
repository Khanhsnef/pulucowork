...
function SmartClaude {
    ...
    if ($lower -match "phân tích|chiến lược|kế hoạch|logic|kiến trúc|hệ thống|quy hoạch|tư duy|chiều sâu|đánh đổi|trade.off|p&l|sla|nguyên nhân|root cause|insight|quyết định|decision|rủi ro|fraud|cung cầu|supply|demand|tâm lý|hành vi") {
        ...
        Write-Host "`n🧠 [Smart Router] Task Tư Duy Sâu → OPUS 4.8 (Max Logic)" -ForegroundColor Magenta
    ...
    # ── Tier 3: DEEPSEEK FLASH — Hỏi đáp nhanh, tính toán nhẹ ─────
        ...
        Write-Host "`n💨 [Smart Router] Task Nhanh → DEEPSEEK V4 FLASH (Siêu Tốc)" -ForegroundColor Yellow
    ...
    # ── Tier 4: SONNET — Code, format, báo cáo, UI ──────────────────
        ...
        Write-Host "`n💻 [Smart Router] Task Code/Format → SONNET 4.6 (Max Coding)" -ForegroundColor Green

    # ── Default ──────────────────────────────────────────────────────
    } else {
        Write-Host "`n🤖 [Smart Router] Task chung → SONNET 4.6 (Mặc định)" -ForegroundColor Blue
    }
...
function SmartChat {
    ...
    if ($lower -match "phân tích|chiến lược|kế hoạch|logic|kiến trúc|hệ thống|quy hoạch|tư duy|chiều sâu|đánh đổi|trade.off|p&l|sla|nguyên nhân|root cause|insight|quyết định|decision|rủi ro|fraud|cung cầu|supply|demand|tâm lý|hành vi") {
        ...
        $model = "cc/claude-sonnet-4-6"
        Write-Host "`n💻 [Smart Router] Task Code/Format → SONNET 4.6" -ForegroundColor Green
    } else {
        Write-Host "`n🤖 [Smart Router] Task chung → SONNET 4.6 (Mặc định)" -ForegroundColor Blue
    }
...
Write-Host "🚀 PuluSmartFlow v3.0 Loaded | Aliases: chat, ai, c-think, c-code, c-fast, 9router, use-9router" -ForegroundColor DarkGray
